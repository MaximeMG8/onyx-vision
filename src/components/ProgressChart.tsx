...
const ProgressChart = ({
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    if (e.touches.length === 2) {
      ...
      lastPinchDistance.current = Math.hypot(touch2.clientX - touch1.clientX, touch2.clientY - touch1.clientY);
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      return <div className="px-2.5 py-1.5 rounded backdrop-blur-sm" style={{
  ...
  if (allChartData.length === 0) {
    return <div className="h-[160px] flex items-center justify-center">
  ...
  return <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        ...
        {zoomLevel > 1 && <motion.span initial={{
        opacity: 0,
        scale: 0.8
      }} animate={{
        opacity: 1,
      ...
      <div ref={containerRef} className="h-[170px] w-full relative touch-none" style={{
      background: 'transparent'
    }} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onClick={handleTap}>
        {/* Zoom hint */}
        {zoomLevel === 1 && allChartData.length > 5 && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
        delay: 1
      }} className="absolute top-2 right-2 z-10 opacity-50 my-[115px]">
        ...
        {selectedPoint && <motion.div initial={{
        opacity: 0,
        y: -10
      }} animate={{
        opacity: 1,
        ...
        opacity: 0
      }} className="absolute top-3 left-1/2 -translate-x-1/2 z-20 border px-4 py-2 rounded-lg backdrop-blur-md" style={{
        backgroundColor: 'hsla(0 0% 100% / 0.95)',
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={visibleData} margin={{
          top: 5,
          right: 5,
          left: -15,
          bottom: 0
            ...
            fontWeight: 300,
            fontFamily: 'system-ui, sans-serif'
          }} tickFormatter={value => value >= 1000 ? `€${(value / 1000).toFixed(1)}k` : `€${value}`} width={40} tickCount={6} />
            <Tooltip content={<CustomTooltip />} />
            <Area type="monotone" dataKey="total" stroke="hsl(0 0% 100%)" strokeWidth={2} fill={`url(#progressGradient-${accentColor.replace(/\s/g, '')})`} style={{
            filter: 'drop-shadow(0 0 8px hsla(0 0% 100% / 0.4))'
          }} animationDuration={300} />
            {selectedPoint && <ReferenceDot x={selectedPoint.date} y={selectedPoint.total} r={6} fill={accentHsl} stroke="white" strokeWidth={2} />}
          </AreaChart>
        </ResponsiveContainer>

        {/* Pan indicator when zoomed */}
        {zoomLevel > 1 && <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex gap-1">
            {Array.from({
          length: Math.min(5, Math.ceil(zoomLevel))
        }).map((_, i) => {
          ...
          return <div key={i} className="h-1 rounded-full transition-all duration-200" style={{
      ...
      {/* Zoom instructions */}
      {zoomLevel > 1 && <p className="text-center text-[10px] text-muted-foreground/50 mt-2 uppercase tracking-wider">
